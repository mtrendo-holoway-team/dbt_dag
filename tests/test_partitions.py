...
def test_partition_warehouse_reader_returns_latest_rows_by_partition(tmp_path: Path) -> None:
    ...
    rows = reader.fetch_latest_partition_snapshot(_model_node())
    ...
    assert "stg__dbt_run_partition_info" in adapter.run_query.call_args_list[0].args[0]
    assert "model.demo.stg_orders" in adapter.run_query.call_args_list[0].args[0]
...
def test_partition_repository_persists_partition_updated_at(tmp_path: Path) -> None:
    repository = _repository(tmp_path)
    ...
    repository.replace_snapshot(
        ...
        [
            PartitionSnapshotRow(
                ...
                row_count=10,
                source_inserted_at=datetime(2026, 6, 4, 10, tzinfo=MSK),
                partition_updated_at=datetime(2026, 6, 4, 8, tzinfo=MSK),
            )
        ],
    )
...
def test_partition_service_uses_min_partition_updated_at_per_day(tmp_path: Path) -> None:
    repository = _repository(tmp_path)
    repository.replace_snapshot(
        "model.demo.stg_orders",
        [
            PartitionSnapshotRow(
                partition_key="2026-06-01-a",
                partition_date=date(2026, 6, 1),
                row_count=10,
                source_inserted_at=datetime(2026, 6, 4, 10, tzinfo=MSK),
                partition_updated_at=datetime(2026, 6, 4, 9, tzinfo=MSK),
            ),
            PartitionSnapshotRow(
                partition_key="2026-06-01-b",
                partition_date=date(2026, 6, 1),
                row_count=30,
                source_inserted_at=datetime(2026, 6, 4, 10, tzinfo=MSK),
                partition_updated_at=datetime(2026, 6, 4, 7, tzinfo=MSK),
            ),
        ],
    )
    warehouse_reader = Mock()
    ...
    assert calendar.months[0].days[0].partition_updated_at == datetime(2026, 6, 4, 7, tzinfo=MSK)


def test_partition_warehouse_reader_reads_partition_updated_at_when_available(
    tmp_path: Path,
) -> None:
    adapter = Mock()
    adapter.adapter_kind = AdapterKind.BIGQUERY
    adapter.run_query.side_effect = [
        ...
        QueryResult(columns=["1"], rows=[{"f0_": 1}]),
        QueryResult(
            columns=["partition_date", "partition_updated_at"],
            rows=[
                {
                    "partition_date": date(2026, 6, 1),
                    "partition_updated_at": datetime(2026, 6, 4, 8, tzinfo=MSK),
    ...
    assert rows[0].partition_updated_at == datetime(2026, 6, 4, 8, tzinfo=MSK)


def test_partition_warehouse_reader_skips_partition_updated_at_when_column_missing(
    tmp_path: Path,
) -> None:
    adapter = Mock()
    adapter.adapter_kind = AdapterKind.BIGQUERY
    adapter.run_query.side_effect = [
        ...
        QueryResult(columns=["1"], rows=[]),
    ]

    reader = BigQueryPartitionWarehouseReader(adapter, _runtime_profile(tmp_path))
    rows = reader.fetch_latest_partition_snapshot(_model_node())

    assert rows[0].partition_updated_at is None
...
def _model_node() -> DbtManifestNode:
    return DbtManifestNode(
        ...
        raw={
            ...
            "schema": "analytics",
            "alias": "stg_orders",
            "config": {"partition_by": {"field": "event_date"}},
        },
    )
